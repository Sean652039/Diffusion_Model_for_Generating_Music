...
def create_piano_roll_matrix(
        ...
        min_pitch: int = 17,
        max_pitch: int = 112  # The range from the dataset is 21 to 108, but we need to expand to 17 to 112 to make it a multiple of 16
...
if __name__ == '__main__':
    ...
    output_dir = pathlib.Path('piano_roll_images')
    ...
    for filename in tqdm(filenames):
        ...
        num_jpg = piano_roll.shape[1] // 4096
        for i in range(num_jpg):
            picture = piano_roll[:, i*4096:(i+1)*4096]
            picrure_raw = np.zeros((768, 512), dtype=np.uint8)
            count = 0
            for j in range(8):
                for k in range(8):
                    picrure_raw[j*96:(j+1)*96, k*64:(k+1)*64] = picture[:, count*64:(count+1)*64]
                    count += 1
            picture_raw = (1 - picrure_raw) * 255
